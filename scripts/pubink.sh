#!/bin/bash
OUTPUT="$(snmpwalk -v1 -c public $4 iso.3.6.1.2.1.43.11.1.1.9 | sed 's|.*:\ ||g')"
BLACK=$(echo -e "$OUTPUT" | sed -n '4p')
YELLOW=$(echo -e "$OUTPUT" | sed -n '3p')
CYAN=$(echo -e "$OUTPUT" | sed -n '2p')
MAGENTA=$(echo -e "$OUTPUT" | sed -n '1p')
